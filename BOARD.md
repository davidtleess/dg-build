...
| DG-147 | A rookie the league did not draft gets no number and no "Rookie" word on the roster, while his card scores him | **3** | doing — Bob (`davidleess-08`); built `f1da0029` on `ticket/DG-147`: index admits any rostered ENGINE_A row (255→263, the 8 names), `is_prospect` = Sleeper `years_exp == 0`, `draft_class` served; backend 6,848 green; adversarial review running; lands before DG-149 | — |
| DG-149 | Two team signals on every player, in different spots: NFL team (FA when none) and league team (FA when unowned) — David 09-04 07:35 ET | **6** | doing — Bob (`davidleess-08`); built `1ac4ab49` on `ticket/DG-149`: API serves the NFL team as a fact, "FA" minted once in copy.ts for header/roster row/movers; `league_ownership.team_name` from the league snapshot (9 of 12 managers named a team); backend 6,853 + gate 647 green; review running; lands after DG-147 | — |
