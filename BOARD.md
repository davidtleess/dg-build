...
| DG-142 | The roster page says three positions "passed accuracy checks" that were never run on the served models — the comparison is a generic label, so the stale branch is unreachable | **2** | open — filed 09-03 06:4x by Tower from Bob's review, verified end-to-end on the live API (`check_served_alignment` = aligned False on 4 of 4). The correct guard EXISTS at `served_model_alignment.py:51`, wired into `trust_surface.py:130` but NOT `roster_audit_models.py`. **Land AFTER David's ~09:15 read — the honest result is all four read "not proven" + a degraded banner until the backtests are re-run.** | unclaimed |
