...
| DG-090 | Daily tape 5px desktop overflow + 46-node contrast debt (pre-existing, exposed by the repaired harness) | **6** | SPLIT by David 08-29 night: Problem A (overflow) pulled to tonight; Problem B (contrast) folds into DG-091's visual pass | — |
| DG-091 | First-user verdict: frontend "not great" — all four dimensions (looks, density, language, navigation) | **6** | brief drafting pulled to 08-29 night on David's pick; VENUE RULED: Studio fresh-eyes (via Tower, wall respected); build post-freeze / season wk 1-2 | — |
...
| DG-094 | POST /api/trade/analyze returns a trade winner — Ruling 07 violation live on trunk (trade_analyzer.py:231) | **6** | todo — pre-freeze buffer (hours): delete delta_status + compute_delta_status, rewrite 5 contract tests to assert absence | — |
| DG-095 | Actions ranked by an xVAR scalar — Ruling 10 violation (taxi sort :518-519, cut_priority :359/:375, partner_score :219) | **5** | todo — pre-freeze stretch, else week-1 slot after DG-065; forced (0) and exempt (-1) sentinels both preserved, only ordinal ranks go | — |
...
| DG-103 | decision_supported criteria + outcome finality — standing proposal drafted; both blanks bite at December's Gate-4 read | **5** | todo — RATIFICATION IS DAVID'S (MASTER §8.5); proposal text in the ticket | — |
