...
| DG-135 | `frontend/openapi.json` does not declare the 503 that `/api/engine-b/scores` and `/api/roster/audit` return since DG-133 | **3** | **built on `ticket/DG-135` 09-02 15:10 (Tower), dry-run green — LANDING PENDING.** Premise was FALSE: regen produced a zero-line diff because the 503 was raised at runtime and never declared. Fix = `responses={503: …}` on both routes with models describing the `{"detail": {…}}` envelope FastAPI actually sends, then regen: openapi diff exactly the two 503s + four schemas; generated TS/Zod client regenerated with it. 5 contract tests (real 503 body validates against the declared model). Exposed DG-138. Needs bundle rebuild at pull (generated client changed). | Tower |
...
| DG-138 | Three routes (`roster_capacity`, `model_scoreboard`, `realized_outcome_scorecard`) declare a flat 503 model while the wire carries `{"detail": {…}}` — the four `JSONResponse` routes are honest | **3** | open — filed 09-02 15:15 by Tower from DG-135; no live surface reads the body (frontend gates on status), so post-freeze is fine; recommended fix = return `JSONResponse` like the honest four, regen must be a zero diff | unclaimed |
